...
const MainMenu = ({ menuTitle, items }: MainMenuProp) => {
  return (
    <div className="rounded-xl border border-gray-700 shadow-md">
      ...
      <div className="flex flex-col mt-12 p-2 gap-3 pb-4">
        {items.map((items, index) => {
          return (
            <div key={index} className="flex items-center justify-between">
              <span className="text-base   z-10">{items.name}</span>
             <div className="flex-1 border-b-2 border-dotted border-gray-500 mx-3" />
              <div
                className={`${iranSans.className} flex items-center justify-center gap-2 `}
              >
                <span className="bg-pink-100 border border-pink-500 px-1 text-gray-600 rounded-md">
                  {items.twoRoolPrice}
                </span>
                <span className="bg-pink-100 border border-pink-500 px-1 text-gray-600 rounded-md">
                  {items.threeRoolPrice}
                </span>
              </div>
            </div>
          );
